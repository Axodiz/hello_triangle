...
float mouseXPos = SCR_WIDTH / 2.0f;
float mouseYPos = SCR_HEIGHT / 2.0f;
...
int main()
{
    ...
    unsigned char *data = stbi_load("textures/diffuse_map.png", &width, &height, &nrChannels, 0);
    if (data)
    {
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    // load image, create texture and generate mipmaps
    data = stbi_load("textures/diffuse_map.png", &width, &height, &nrChannels, 0);
    if (data)
    {
    ...
    while (!glfwWindowShouldClose(window))
    {
		...
		camera.update(mouseXPos, mouseYPos);
